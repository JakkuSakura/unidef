...
class VariableDeclarationProcessor(NodeTypeProcessor):
    ...
    def prepare_inference(
        ...
        if val:
            builder.add_edge(
                ns, environment.get_advanced_ir_node(val, scope), EdgeType.EQUAL_TO
            )
...
class AssignExpressionProcessor(NodeTypeProcessor):
    ...
    def prepare_inference(
        ...
        decl = environment.find_declaration(scope, left)
        builder.add_edge(
            decl,
            node2=environment.get_advanced_ir_node(right, scope),
            edge_type=EdgeType.EQUAL_TO,
        )
...
class ReturnProcessor(NodeTypeProcessor):
    ...
    def prepare_inference(
        ...
        builder.add_edge(
            ns, environment.get_advanced_ir_node(ret, scope), EdgeType.EQUAL_TO
        )


class MemberExpressionProcessor(NodeTypeProcessor):
    def accept(self, node: IrNode) -> bool:
        return node.get_field(Attributes.MemberExpression)
    ...
    def prepare_inference(
        ...
        builder.add_edge(
            ns, self.environment.get_advanced_ir_node(obj, scope), EdgeType.EQUAL_TO
        )
        builder.add_edge(
            ns, self.environment.get_advanced_ir_node(prop, scope), EdgeType.EQUAL_TO
        )
...
class FunctionDeclProcessor(NodeTypeProcessor):
    ...
    def prepare_inference(
        ...
        for arg in args:
            arg_path = str(environment.find_global_path(scope, arg))
